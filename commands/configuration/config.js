...
module.exports = {
	...
	do: async (locale, message, client, args, Discord) => {
		...
		},
		{
			...
			cfg: async function() {
				if (!args[1]) return message.channel.send(string(locale, qServerDB.config.anon ? "CFG_ANONYMOUS_ENABLED" : "CFG_ANONYMOUS_DISABLED", { invite: slash_url.replace("[ID]", client.user.id) }));
				switch (args[1].toLowerCase()) {
				case "enable":
				case "on": {
