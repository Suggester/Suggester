...
module.exports = {
	...
	do: async (message, client, args, Discord) => {
		...
		if (reasonSplit[1]) {
			...
			if (reason.length > 1024) return message.channel.send(`<:${config.emoji.x}> Denial reasons cannot be longer than 1024 characters.`);
		}
		...
		if (n !== 0) {
			await msg.edit(
				new Discord.RichEmbed()
					.setDescription(`<:${config.emoji.check}> ${nModified !== 0 ? "Successfully denied" : "Denied"} ${nModified}/${su.length} suggestions`)
			...
			return await msg.edit(
				new Discord.RichEmbed()
					.setDescription(`<:${config.emoji.x}> None of the suggestions you provided could be denied. Please make sure the suggestion IDs you have provided exist and have not already been denied.`)
					.setColor("#e74c3c")
			);
		}
			...
			if (denied.hasOwnProperty(s)) {
				...
				if (qServerDb.config.channels.log) {
					let logEmbed = new Discord.RichEmbed()
						.setAuthor(`${message.author.tag} denied #${denied[s].suggestionId}`, message.author.displayAvatarURL)
				...
				if (qServerDb.config.channels.denied) {
					...
					client.channels.get(qServerDb.config.channels.denied)
						.send(deniedEmbed);
				}
