...
module.exports = {
	...
	do: async (locale, message, client, args, Discord) => {
		...
		if (args[0].toLowerCase() === "list") {
			if (qServerDB.config.blacklist.length < 1) return message.channel.send(string(locale, "BLACKLIST_EMPTY"));
		...
		if (user.id === message.author.id) return message.channel.send(string(locale, "BLACKLIST_SELF_ERROR", {}, "error"));
		if (user.bot) return message.channel.send(string(locale, "BLACKLIST_USER_BOT_ERROR", {}, "error"));

		let qUserDB = await dbQuery("User", {id: user.id});

		await message.guild.members.fetch(user.id).catch(() => {});

		if (qServerDB.config.blacklist.includes(user.id)) return message.channel.send(string(locale, "ALREADY_BLACKLISTED_ERROR", {}, "error"));
		if (qUserDB.flags.includes("STAFF")) return message.channel.send(string(locale, "BLACKLIST_GLOBAL_STAFF_ERROR", {}, "error"));
		if (message.guild.members.cache.get(user.id)) {
			let memberPermission = await checkPermissions(message.guild.members.cache.get(user.id), client);
			if (memberPermission <= 2) return message.channel.send(string(locale, "BLACKLIST_STAFF_ERROR", {}, "error"));
		}
		...
		message.channel.send(`${string(locale, "BLACKLIST_SUCCESS", { user: user.tag, id: user.id }, {}, "check")}${reason ? `\n${string(locale, "BLACKLIST_REASON_HEADER")} ${reason}` : ""}`, { disableMentions: "all" });
