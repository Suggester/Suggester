...
module.exports = {
	...
	do: async (message, client, args, Discord) => {
		...
		if (qServerDB.config.notify && qUserDB.notify) {
			...
			if(qServerDB.config.selfnotify===false && suggester.id!==message.author.id) suggester.send(dmEmbed).catch(err => console.log(err));
			if(qServerDB.config.selfnotify) suggester.send(dmEmbed).catch(err => console.log(err));
		}
